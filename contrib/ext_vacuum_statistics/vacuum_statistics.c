...
static bool extvac_lookup_relation(Oid relid, char *relkind,
								   bool *relisshared);
...
/*
 * Map a relkind to the entry type we key relation statistics by.
 *
 * Returns PGSTAT_EXTVAC_INVALID for relkinds vacuum never reports on, so
 * that they are skipped without touching the statistics hash.
 */
static ExtVacReportType
extvac_type_for_relkind(char relkind)
{
	switch (relkind)
	{
		case RELKIND_INDEX:
			return PGSTAT_EXTVAC_INDEX;
		case RELKIND_RELATION:
		case RELKIND_MATVIEW:
		case RELKIND_TOASTVALUE:
			return PGSTAT_EXTVAC_TABLE;
		default:
			return PGSTAT_EXTVAC_INVALID;
	}
}

/*
 * Object access hook: keep the statistics hash in step with the catalogs.
 *
 * Without this, an entry outlives the relation it describes: it keeps
 * occupying shared memory, gets serialized into the statistics file on
 * shutdown and read back on startup, and - once its OID is handed to a new
 * relation - starts accumulating that relation's vacuums on top of the dead
 * one's counters.  Both operations are registered transactionally, so they
 * take effect only if the surrounding transaction commits, just like the
 * built-in relation statistics.
 *
 * Databases need no handling here: dropping the built-in database entry
 * drops every entry of that database, custom kinds included.
 */
/*
 * Look up the relkind and shared-ness of a relation in pg_class.
 *
 * At OAT_POST_CREATE the new pg_class row has not been made visible by a
 * CommandCounterIncrement() yet, so a syscache lookup such as
 * get_rel_relkind() would not find it.  Scan the catalog with SnapshotSelf
 * instead, as sepgsql does for the same reason.  At OAT_DROP the row has
 * not been deleted yet, so it is found that way as well.
 *
 * Returns false if there is no such relation.
 */
static bool
extvac_lookup_relation(Oid relid, char *relkind, bool *relisshared)
{
	Relation	rel;
	ScanKeyData skey;
	SysScanDesc sscan;
					 ...
					 int subId, void *arg)
{
	...
	if (!extvac_lookup_relation(objectId, &relkind, &relisshared))
		return;

	type = extvac_type_for_relkind(relkind);
	if (type == PGSTAT_EXTVAC_INVALID)
		return;

	dboid = relisshared ? InvalidOid : MyDatabaseId;

	if (access == OAT_DROP)
		pgstat_drop_transactional(PGSTAT_KIND_EXTVAC_RELATION, dboid,
								  EXTVAC_OBJID(objectId, type));
	else
	{
		/*
		 ...
		 */
		pgstat_create_transactional(PGSTAT_KIND_EXTVAC_RELATION, dboid,
									EXTVAC_OBJID(objectId, type));
	}
}
