...
accumulate_heap_vacuum_statistics(LVRelState *vacrel, PgStat_VacuumRelationCounts * extVacStats)
{
	...
	/*
	 * Map LVRelState's three independent VM counters onto the extended
	 * statistics fields:
	 *   vm_new_visible_pages         - newly set all-visible (not frozen)
	 *   vm_new_visible_frozen_pages  - newly set all-visible AND all-frozen
	 *                                  in the same VACUUM pass
	 *   vm_new_frozen_pages          - pages set all-frozen that were
	 *                                  already all-visible coming in
	 */
